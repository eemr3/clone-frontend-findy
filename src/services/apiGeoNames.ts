...
export async function getCities(city: string) {
  const response = await apiCities.get<PostalCodes>(
    `/postalCodeSearchJSON?placename_startsWith=${city}&maxRows=${
      city.length <= 3 ? 500 : 100
      ...
      (newCity, index, alias) =>
        alias.findIndex(
          (cityFound) =>
            ...
            newCity.placeName === cityFound.placeName,
        ) === index,
    )
    .map((newCity) => {
