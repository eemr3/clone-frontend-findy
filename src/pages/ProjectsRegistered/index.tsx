...
export function ProjectRegistred() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const languages = await Promise.all(response.data.map(async (project: any) => {
  
        const languageIds = project.language;
        const languagePromises = languageIds.map((stackId: any) => getLanguagesById(stackId?.stackId));
...
/*   async function getLanguageNames(projectId: number): Promise<string[]> {
  ...
  return (
    <section className="w-max-[144rem] flex  flex-col  bg-blue-dark">
      ...
      <article className="flex justify-center items-center min-h-[max-context] flex-col bg-blue-dark pl-[15.9rem] pt-[6.414rem] text-grey-#5 lg:px-[3rem] mbl:pb-[5rem] mbl:justify-center">
        <Heading
          type="lg-leading58"
          className="text-[5.5rem] xl:text-[5rem] lg:text-[4rem] mbl:text-[2.2rem] leading-[5rem] "
        >
        ...
        <Heading
          type="lg-leading58"
          className="mt-[8rem] text-[4.5rem]  xl:text-[4rem] lg:text-[2.5rem]  mbl:text-[1.8rem] leading-[6rem]"
        >
          Projetos cadastrados
        </Heading>

        <Text type="md" className=" inline-block xl:text-[2rem] lg:text-[1.5rem] mbl:text-[1.4rem] ">
        ...
        <button className="mt-[5rem] h-[5rem] w-[60%] max-w-[40.6rem] rounded-[3.2rem] bg-[#01A195] px-[2.5rem]  mbl:h-[4rem] mbl:w-[75%] mbl:px-[0.1rem]  mbl:text-[1rem]">
          <Link to="/project" >
            <p className="text-[2.2rem] text-[#FFFFFF]  lg:text-[1.5rem]">ADICIONAR NOVO PROJETO</p>
            
          </Link>
        </button>
      </article>

      <section className=" mt-[10.2rem]  flex flex-col gap-[3rem] bg-[#FFFFFF] pb-[10rem] pt-[8rem] xl:px-[2rem] md:px-[2rem]  mbl:  mbl:px-[1.5rem]  mbl:pt-[2rem] ">
        <div className="mx-auto flex h-[100%] w-[100%] flex-col items-center  gap-[3rem] ">
          {projects?.map((project: Projects,index) => {
            return (
              <div key={index} className="mx-auto flex h-[100%] h-[100%] w-[100%] max-w-[120rem] items-center shadow-shadow-#1">
                ...
                <div className="w-[100%] pb-[1rem] pl-[11.5rem] pr-[6.7rem] pt-[3rem] font-bold md:pl-[4rem] mbl:pl-[2rem] mbl:pr-[1rem]">
                  ...
                  {languageNames[index]?.map((arra: any) => {
                     ...
                     return <Tag label={arra?.data?.title} tagColor={"green"} ></Tag>
